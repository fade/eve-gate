...
(defparameter *scheduler-priority-map*
  `((:critical  . ,+priority-critical+)
    ...
    (:cold      . ,+priority-low+)
    (:bootstrap . ,+priority-bulk+))
  "Mapping from consumer-facing priority keywords to engine priority integers.

`:bootstrap` is present for internal bookkeeping (handle dispatch via the
engine and held-queue slot indexing both use this map). It is NOT accepted
as a `:priority` argument to `submit-refresh` — that path explicitly rejects
`:bootstrap` and directs callers to `submit-bootstrap`, which constructs
bootstrap handles directly and routes them through the bootstrap-paced
dispatcher branch.")
...
HOT matches the 5-minute heat-map cadence; WARM matches 30 min; COLD and
BOOTSTRAP match 6 h. Consumers can override per submission.")
...
(defstruct (bootstrap-handle (:constructor %make-bootstrap-handle))
  "Opaque group handle returned by `submit-bootstrap`. Aggregates a batch
of per-spec refresh-handles and tracks settled / outcome counts so consumer
code and `scheduler-state` can observe progress without filtering the
held-queue.

Concurrent updates from per-spec callbacks land under LOCK. The counters
reflect terminal outcomes only: a spec contributes exactly once when its
underlying refresh-handle delivers a completion (ok, cache-hit, skipped,
or failed). COMPLETED-COUNT is the settled count (total terminal); the
OK / FAILED / SKIPPED counters partition it by outcome class.

Slots:
  ID                 Symbol gensym for logging and the scheduler's
                     bootstrap-batches index.
  SCHEDULER          Back-reference to the parent request-scheduler.
  BATCH-ID           Pass-through grouping string. Auto-generated by
                     submit-bootstrap when the caller does not supply one.
  REFRESH-HANDLES    List of the per-spec refresh-handles, in submission
                     order. Used by cancel-bootstrap to mark each one.
  TOTAL              Number of specs in the batch (set at submit time).
  COMPLETED-COUNT    Number of specs that have reached a terminal outcome
                     (any of ok, cache-hit, skipped, failed). Drives the
                     `:pending` field of scheduler-state.
  OK-COUNT           Subset of COMPLETED-COUNT covering :ok and :cache-hit.
  FAILED-COUNT       Subset of COMPLETED-COUNT covering :failed.
  SKIPPED-COUNT      Subset of COMPLETED-COUNT covering :skipped.
  CANCELLED-P        Set by cancel-bootstrap. The dispatcher still drops
                     each pending refresh-handle individually; this flag
                     is the batch-level summary.
  ON-PROGRESS        Optional (settled total) callback, fired after each
                     spec settles.
  ON-COMPLETE        Optional (bootstrap-handle) callback, fired once when
                     COMPLETED-COUNT reaches TOTAL.
  LOCK               Guards the counters and CANCELLED-P transitions."
  (id (gensym "BOOTSTRAP-") :type symbol)
  (scheduler nil)
  (batch-id "" :type string)
  (refresh-handles nil :type list)
  (total 0 :type (integer 0))
  (completed-count 0 :type (integer 0))
  (ok-count 0 :type (integer 0))
  (failed-count 0 :type (integer 0))
  (skipped-count 0 :type (integer 0))
  (cancelled-p nil :type boolean)
  (on-progress nil :type (or null function))
  (on-complete nil :type (or null function))
  (lock (bt:make-lock "bootstrap-handle-lock")))
...
esi-rate-limiter, and cache-manager. It owns:
  ...
  - The bootstrap pacing bucket and active bootstrap-handle index for
    bulk drains.
...
Slots:
  ...
  BOOTSTRAP-POOL      Reserved nil. The proposal's dedicated worker-pool
                      was simplified out in favour of routing bootstrap
                      dispatch through the existing dispatcher and
                      held-queue slot 4. Retained so future implementations
                      can opt into a dedicated pool without struct churn.
  BOOTSTRAP-QUEUE-DEPTH  Cap for the bootstrap held-queue (slot 4).
                         submit-bootstrap signals SCHEDULER-QUEUE-FULL if a
                         batch would push slot-4 past this cap.
  BOOTSTRAP-PACING       Bootstrap drain rate cap in requests per second.
                         Used as the refill-rate of BOOTSTRAP-PACING-BUCKET.
  BOOTSTRAP-PACING-BUCKET  Token bucket gating bootstrap dispatch. The
                           dispatcher acquires one token per bootstrap
                           dispatch, bounding throughput to BOOTSTRAP-PACING.
  BOOTSTRAP-BATCHES      Hash-table of bootstrap-handle-id → bootstrap-handle
                         for every active submit-bootstrap call. Consulted
                         by cancel-bootstrap and scheduler-state.
  BOOTSTRAP-BATCHES-LOCK Guards BOOTSTRAP-BATCHES.
  ...
  (bootstrap-pacing-bucket nil)
  (bootstrap-batches (make-hash-table :test 'eq) :type hash-table)
  (bootstrap-batches-lock (bt:make-lock "scheduler-bootstrap-batches-lock"))
  ...
  ;; Slot 4 (bootstrap) is additionally paced by BOOTSTRAP-PACING-BUCKET and
  ;; drained from the head only — see drain-bootstrap-slot.
...
(defun drain-bootstrap-slot (scheduler now)
  "Drain held-queue slot 4 (bootstrap) from the head as long as the head
is dispatchable. Called from drain-held-queues-once with the held-queues
lock held.

Per-iteration the dispatcher pops the head if it is:
  - cancelled (drop silently — lazy cancellation cleanup),
  - past its deadline (deliver an esi-budget-exhausted skip),
  - dispatchable (budget gate open AND a token from the bootstrap pacing
    bucket); the head is popped and dispatched via the engine.

If the head is not dispatchable for token-unavailable or gate-closed
reasons, the function returns — every subsequent FIFO entry has the same
problem, so per-tick work is O(K) where K is the number of tokens
acquired (with rare additive cost for consecutive cancelled / expired
heads). This keeps the dispatcher's per-tick cost bounded regardless of
the slot-4 depth.

Slot 4 is treated as gate-equivalent to :warm for the budget check: the
gate-state is global, so the priority argument to gate-open-for-priority-p
just controls the :critical bypass."
  (let ((bucket (request-scheduler-bootstrap-pacing-bucket scheduler)))
    (loop
      (let ((q (aref (request-scheduler-held-queues scheduler)
                     +priority-bulk+)))
        (when (null q)
          (return))
        (let* ((entry (first q))
               (handle (getf entry :handle))
               (status (refresh-handle-status handle))
               (deadline (refresh-handle-deadline-at handle)))
          (cond
            ((eq status :cancelled)
             (setf (aref (request-scheduler-held-queues scheduler)
                         +priority-bulk+)
                   (rest q)))
            ((> now deadline)
             (setf (aref (request-scheduler-held-queues scheduler)
            ...
            ((not (gate-open-for-priority-p scheduler :warm))
             (return))
...
Slots 0-3 (critical/hot/warm/cold) are iterated in full each sweep — their
depth is bounded by the heat-map cadence and the duration of any
gate-closure window, not by bulk volume. For each entry in FIFO order:
  - drop it silently if the handle has been cancelled,
  - deliver an esi-budget-exhausted skip if its deadline has passed,
  - dispatch it via the engine if the gate is open for its priority,
  - otherwise leave the entry in place for the next sweep.

Slot 4 (bootstrap) is drained from the head only by drain-bootstrap-slot —
once the head is non-dispatchable on the token-or-gate check, the
dispatcher stops walking. Per-tick work is O(K) where K is the number of
tokens acquired, independent of the slot's depth."
  (let ((now (get-universal-time)))
    (bt:with-lock-held ((request-scheduler-held-queues-lock scheduler))
      (loop for priority from 0 below +priority-bulk+ do
        (let ((q (aref (request-scheduler-held-queues scheduler) priority))
          ...
          (setf (aref (request-scheduler-held-queues scheduler) priority)
                (nreverse keep))))
      (drain-bootstrap-slot scheduler now))))
...
Returns a request-scheduler. Call start-scheduler to bring it online."
  ...
  (let* ((limiter (or rate-limiter (ensure-rate-limiter)))
         ...
         (pacing-bucket
           (make-token-bucket
            :name "scheduler-bootstrap-pacing"
            :refill-rate (float bootstrap-pacing)
            :max-tokens (float (max 1.0 bootstrap-pacing)))))
    (%make-request-scheduler
     ...
     :bootstrap-pacing (float bootstrap-pacing)
     :bootstrap-pacing-bucket pacing-bucket
     :failure-ring ring
...
(defun bump-bootstrap-counters (b-handle completion)
  "Update BOOTSTRAP-HANDLE counters from a per-spec completion and fire the
batch-level on-progress / on-complete callbacks under the bootstrap-handle's
lock. The callback parameters are read under the lock so that progress
callbacks fire with a coherent (settled total) view."
  (let (settled total on-progress on-complete-cb done-p)
    (bt:with-lock-held ((bootstrap-handle-lock b-handle))
      (incf (bootstrap-handle-completed-count b-handle))
      (case (refresh-completion-outcome completion)
        ((:ok :cache-hit) (incf (bootstrap-handle-ok-count b-handle)))
        (:failed          (incf (bootstrap-handle-failed-count b-handle)))
        (:skipped         (incf (bootstrap-handle-skipped-count b-handle))))
      (setf settled (bootstrap-handle-completed-count b-handle)
            total (bootstrap-handle-total b-handle)
            on-progress (bootstrap-handle-on-progress b-handle)
            on-complete-cb (bootstrap-handle-on-complete b-handle)
            done-p (= settled total)))
    (when on-progress
      (handler-case (funcall on-progress settled total)
        (error (e)
          (log-error "Bootstrap on-progress callback errored: ~A" e))))
    (when (and done-p on-complete-cb)
      (handler-case (funcall on-complete-cb b-handle)
        (error (e)
          (log-error "Bootstrap on-complete callback errored: ~A" e))))))

(defun submit-bootstrap (scheduler specs
                         &key batch-id
                              deadline-seconds
                              on-progress
                              on-complete)
  "Submit a batch of bulk refreshes for a one-shot cold-start drain.

SPECS is a list of plists. Each plist accepts the same shape as
SUBMIT-REFRESH's keyword arguments minus :priority — bootstrap priority is
implicit and per-spec :priority entries are ignored. Accepted per-spec
keys: :operation-id, :params, :on-complete, :on-skip, :on-fail.

The batch as a whole is paced at the scheduler's BOOTSTRAP-PACING rate
(default 5 req/sec). Bootstrap entries only dispatch when both the budget
gate is open AND a token from the scheduler's BOOTSTRAP-PACING-BUCKET is
available. They land on held-queue slot 4 and are drained from the head
by the dispatcher.

BATCH-ID            Opaque string for grouping completions in observability;
                    auto-generated if not supplied.
DEADLINE-SECONDS    Per-spec dispatch deadline; defaults to 21600 (6h).
ON-PROGRESS         (settled total) → void. Fired after each per-spec
                    callback under the bootstrap-handle's lock.
ON-COMPLETE         (bootstrap-handle) → void. Fired once when every spec
                    has settled.

Per-spec callbacks (:on-complete / :on-skip / :on-fail) fire as their
underlying refreshes settle, before the batch-level counters are bumped.

Cache short-circuit fires synchronously for any spec whose endpoint /
params / auth tuple is fresh in the cache-manager. Cache-hits contribute
to OK-COUNT and never consume a pacing token; only non-cached specs go
through the engine and the pacing gate.

Signals SCHEDULER-NOT-RUNNING if the scheduler hasn't been started.
Signals SCHEDULER-QUEUE-FULL if the batch would push slot 4 past
BOOTSTRAP-QUEUE-DEPTH. Signals SCHEDULER-UNKNOWN-ENDPOINT for any spec
whose operation-id does not resolve via find-endpoint.

Returns a BOOTSTRAP-HANDLE. Query its counters with
BOOTSTRAP-HANDLE-COMPLETED-COUNT, BOOTSTRAP-HANDLE-OK-COUNT, etc."
  (ensure-scheduler-running scheduler nil)
  (let* ((total (length specs))
         (effective-batch (or batch-id
                              (format nil "bootstrap-~A" (gensym "B-"))))
         (deadline (or deadline-seconds
                       (default-deadline-for :bootstrap)))
         (b-handle (%make-bootstrap-handle
                    :scheduler scheduler
                    :batch-id effective-batch
                    :total total
                    :on-progress on-progress
                    :on-complete on-complete)))
    ;; Pre-flight: bootstrap queue depth check.
    (let* ((cap (request-scheduler-bootstrap-queue-depth scheduler))
           (slot-4-size
             (bt:with-lock-held ((request-scheduler-held-queues-lock scheduler))
               (length (aref (request-scheduler-held-queues scheduler)
                             +priority-bulk+)))))
      (when (> (+ slot-4-size total) cap)
        (error 'scheduler-queue-full
               :scheduler scheduler
               :operation-id nil
               :priority :bootstrap)))
    ;; Register the batch in the scheduler's bootstrap-batches index.
    (bt:with-lock-held ((request-scheduler-bootstrap-batches-lock scheduler))
      (setf (gethash (bootstrap-handle-id b-handle)
                     (request-scheduler-bootstrap-batches scheduler))
            b-handle))
    ;; Per-spec handle construction.
    (dolist (spec specs)
      (let* ((operation-id (getf spec :operation-id))
             (params (getf spec :params))
             (orig-on-complete (getf spec :on-complete))
             (orig-on-skip (getf spec :on-skip))
             (orig-on-fail (getf spec :on-fail)))
        (multiple-value-bind (meta op-string)
            (resolve-endpoint-or-error operation-id)
          (let* ((path-template (getf meta :path))
                 (method (or (getf meta :method) :get)))
            (multiple-value-bind (path-subs query-pairs)
                (split-path-and-query-params path-template params)
              (let* ((endpoint (substitute-path-parameters path-template path-subs))
                     ...
                     (wrap (lambda (orig)
                             (lambda (completion)
                               (when orig
                                 (handler-case (funcall orig completion)
                                   (error (e)
                                     (log-error
                                      "Bootstrap per-spec callback errored: ~A" e))))
                               (bump-bootstrap-counters b-handle completion))))
                     (handle (%make-refresh-handle
                              :scheduler scheduler
                              :operation-id (or op-string operation-id)
                              :endpoint endpoint
                              ...
                              :status :pending
                              :enqueued-at now
                              :on-complete (funcall wrap orig-on-complete)
                              :on-skip     (funcall wrap orig-on-skip)
                              :on-fail     (funcall wrap orig-on-fail))))
                (push handle (bootstrap-handle-refresh-handles b-handle))
                (register-handle scheduler handle)
                (bump-stat scheduler :submitted)
                ;; Cache short-circuit: deliver synchronously if fresh.
                ;; Cache-hits skip the pacing gate by design — they don't
                ;; consume a token because no HTTP fires.
                (let ((cache-result
                        (if (eq method :get)
                            (consult-cache-for-submission
                             scheduler endpoint query-pairs op-string)
                    ...
                    ((and (consp cache-result) (eq (first cache-result) :hit))
                     (deliver-completion
                      ...
                      (build-completion-from-cache-hit
                       ...
                       now)))
                    (t
                     (hold-submission scheduler handle
                                      method endpoint query-pairs))))))))))
